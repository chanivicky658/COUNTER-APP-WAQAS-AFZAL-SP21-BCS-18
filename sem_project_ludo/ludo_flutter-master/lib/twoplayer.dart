...
class two_player  extends ChangeNotifier {
  ...
  // Fill only yellow and red players
  ...
  void throwDice() async {
    ...
    Future.delayed(const Duration(seconds: 1)).then((value) {
      ...
      // Automatically move random pawn if all pawn are in the same step
      ...
      // If User has a 6 dice, but it is inside the finish line, it will make him throw again, else it will turn to the next player
      if (currentPlayer.pawns.every((element) => !element.highlight)) {
        if (diceResult == 6) {
          throwDice(); // Roll the dice again
          return;
        }
        else {
          nextTurn();
  ...
  // Move pawn to the next step and check if it can kill other pawn
  ...
  // Next turn will be called when the player finishes the turn
  // Next turn will be called when the player finishes the turn
  void nextTurn() {
    switch (_currentTurn) {
      case LudoPlayerType.yellow:
        if (!winners.contains(LudoPlayerType.red)) {
      ...
      case LudoPlayerType.red:
        if (!winners.contains(LudoPlayerType.yellow)) {
          _currentTurn = LudoPlayerType.yellow;
        } else {
          _currentTurn = LudoPlayerType.red;
        }
  ...
  // This function will check if the pawn finished the game or not
